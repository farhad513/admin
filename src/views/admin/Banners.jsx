...
import { GetAmbulances, AmbulanceDelete, messageClear } from "../../store/Reducers/ambulanceReducer";
import toast from "react-hot-toast";
import { Link } from "react-router-dom";

const Banners = () => {
  const dispatch = useDispatch();
  const { ambulances, totalAmbulance, errorMessage, successMessage } = useSelector((state) => state.ambulance);
  ...
  const handleDelete = async (id) => {
    const confirmDelete = window.confirm("তুমি কি এই অ্যাম্বুলেন্স ডিলিট করতে চাও?");
    if (confirmDelete) {
